...
int main(void)
{
	...
	uint32_t count;
	const char** iex = glfwGetRequiredInstanceExtensions(&count);
	...
	GLFWwindow* vk_window = glfwCreateWindow(800, 600, "Window Title", NULL, NULL);

	static VkSurfaceKHR vk_surface = VK_NULL_HANDLE;
	assert(glfwCreateWindowSurface(vk_instance, vk_window, NULL, &vk_surface) == VK_SUCCESS);
